  ...
  <main class="stage">
    ...
    <section class="sec" id="sec-models"><div class="wipe">
      ...
      <!-- Model guidance (merged into Models - Phase 3b): named storms get
           ...
           ONE IMPL, TWO MOUNTS (CYCLOLAB_DESIGN 7.3). The three inline
           guidance renderers this generator used to carry (gTracks / gEns /
           gIntensity) are RETIRED. The guidance viewer is now the shared
           component at triple-a-tropics.com/guidance/guidance.js, lazy-loaded
           on first Models-tab open and locked to this storm - exactly like the
           Recon and HAFS mounts. A second implementation living here is what
           let the two drift apart in the first place.

           What the component adds over the retired renderers: the official
           forecast and the verifying best track drawn distinctly; the OCD5
           no-skill baseline on the intensity chart (without it a guidance
           chart cannot say whether any aid beats climatology-and-persistence);
           a consensus-membership strip in THREE states - present, absent, and
           WITHHELD from the public a-deck, which is the difference between "it
           did not run" and "we are not shown it"; early-vs-late badging; and
           hard basin scoping, so a JTWC-basin storm gets raw ensemble tracks
           only and is never given a consensus envelope it does not have. -->
      <div class="card">
        <h3>Model guidance</h3>
        <div id="guidance-viewer" tabindex="0">
          <div id="guidance-status" class="hafs-statusbox">
            <div class="hafs-spinner"></div><span>Loading model guidance&#8230;</span></div>
        </div>
      </div>
      <div class="card">
...
(function () {
  ...
  // ===================== Model guidance ====================================
  // The guidance VIEWER is the shared component in the main repo
  // (guidance/guidance.js), lazy-loaded and mounted by initGuidance() below -
  // one implementation, mounted here and anywhere else it is needed, the same
  // way the Recon and HAFS tabs work. This generator keeps only the SHIPS
  // panel, which still has its own feed (cyclolab/{SID}/ships.json) and its
  // own renderer until the SHIPS panel moves onto the component too.
  var SHDATA = null, gDrawn = false;
  function gEsc(s) { return String(s == null ? "" : s)
    .replace(/&/g, "&amp;").replace(/</g, "&lt;"); }
  // The three inline guidance renderers that lived here (gTracks / gEns /
  // gIntensity) plus their private helpers - the peak-wind reducer, the
  // guidance basemap, the per-tech hue table, the in-plot info bar, the init
  // stamp, the label stacker and the ensemble colour ramp - are RETIRED. The
  // viewer is now the shared component in the main repo, mounted below by
  // initGuidance(). gEsc and gSpark stay: the SHIPS panel still uses them.
  ...
  function gRenderAll() { try { gShips(); } catch (e3) {} }

  // ---- Guidance mount (SAME component the main repo serves) ---------------
  // Lazy-load GuidanceViewer from the house origin and mount it LOCKED to this
  // storm, exactly like the Recon and ASCAT mounts above. The component owns
  // the spaghetti tracks, the intensity chart with its OCD5 skill baseline,
  // the consensus-membership strip and the early/late badging; it hydrates
  // from cyclolab/{SID}/guidance_v2.json and falls back to the legacy
  // guidance.json. SHIPS stays here for now (its own feed + renderer) and
  // moves onto the component when the SHIPS panel lands.
  var guidanceViewer = null;
  function initGuidance() {
    if (gDrawn) return; gDrawn = true;
    var base = CDN + "/cyclolab/" + encodeURIComponent(SID) + "/";

    // SHIPS: unchanged path, independent of the guidance component.
    fetchJson(base + "ships.json")
      .then(function (s) { SHDATA = s; gRenderAll(); })
      .catch(function () { gRenderAll(); });

    var root = document.getElementById("guidance-viewer");
    var status = document.getElementById("guidance-status");
    if (!root) return;
    function fail() {
      if (status) {
        status.querySelector("span").textContent =
          "Model guidance failed to load - reload to retry.";
      }
    }
    _loadScript(SITE_BASE + "/guidance/guidance.js",
      function () { return !!window.GuidanceViewer; },
      function () {
        try {
          var nmEl = document.getElementById("storm-name");
          guidanceViewer = new window.GuidanceViewer(root, {
            base: CDN + "/cyclolab",
            stormLock: SID,
  ...
  // expose for tests/manual re-render (SHIPS only now)
