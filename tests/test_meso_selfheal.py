...
from poller_framework import SourceHealth      # noqa: E402


def _health(ages: dict):
    """{slug: SourceHealth} where ages maps slug -> seconds-since-last-scan
    (or None = never discovered). Returns (health, now)."""
    now = mp.utcnow()
    h = {}
    for s in mp.MESO_SECTORS:
        sh = SourceHealth(name=s.slug)
        a = ages.get(s.slug, None)
        sh.last_valid_time = None if a is None else now - dt.timedelta(seconds=a)
        h[s.slug] = sh
    return h, now
...
class SelfHealDecide(unittest.TestCase):
    def test_all_stale_fires(self):
        h, now = _health({s.slug: 5000 for s in mp.MESO_SECTORS})  # past any threshold
    ...
    def test_normal_cadence_no_false_trip(self):
        # worst HEALTHY staleness between 120 s discovery refreshes:
        # GOES ~180 s, Himawari ~270 s -- both must read fresh.
    ...
    def test_cold_start_all_none_no_fire(self):
        h, now = _health({s.slug: None for s in mp.MESO_SECTORS})
        self.assertFalse(mp.selfheal_decide(h, now)[0])
